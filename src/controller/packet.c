...
packet_t* packet_deserialize(uint8_t* const p_buffer, int payload_len) {
    ...
    if (payload_len != back->payload_len && payload_len != -1) {
        if (!IS_IN_GRAPHIC_MODE) log_warn("Invalid payload len: Should %i Is %i", payload_len, back->payload_len);
        if (!IS_IN_GRAPHIC_MODE) log_warn("Here is the header:");
        if (!IS_IN_GRAPHIC_MODE) log_warn("Id: %i", back->id);
        if (!IS_IN_GRAPHIC_MODE) log_warn("Token recv: %i", back->token_recv);
        if (!IS_IN_GRAPHIC_MODE) log_warn("Token send: %i", back->token_send);
        if (!IS_IN_GRAPHIC_MODE) log_warn("Type: %i", back->type);
        if (!IS_IN_GRAPHIC_MODE) log_warn("Payload len: %i", back->payload_len);
        free(back);
        return NULL;
    }
...
void packet_print(packet_t* const packet) {
    if (packet != NULL) {
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Id: %i", packet->id);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Token rcv: %i", packet->token_recv);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Token send: %i", packet->token_send);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt type: %i", packet->type);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Payload Len: %i", packet->payload_len);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Checksum: %i", packet->checksum);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Ack: %i ", packet->ack);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Nxt Symbol Rate: %i", packet->next_symbol_rate);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Fallback Rate: %i", packet->fallback_rate);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Second Best Rate: %i ", packet->next_second_best_rate);
        if (!IS_IN_GRAPHIC_MODE) log_debug("Pkt Best Prob Rate: %i", packet->next_highest_pro_rate);
