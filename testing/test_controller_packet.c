...
void test_packet_checksum() {
    ...
    pkt->token_recv = 250;
    pkt->token_send = 400;
    pkt->next_highest_pro_rate = 1;
    pkt->next_second_best_rate = 5;
    pkt->type = 5;
    ...
    assert (pkt->checksum == (65961 + 1 + 5 + 300) % 256); //Manual calculated
...
void testSerializedPacket(packet_t* pkt, uint8_t* serialized) {
    ...
    uint8_t next_second_rate = serialized[count];
    count += 1;
    assert(next_second_rate == pkt->next_second_best_rate);

    uint8_t best_prob_rate = serialized[count];
    count += 1;
    assert(best_prob_rate == pkt->next_highest_pro_rate);
...
void test_packet_serialize_zero() {
    ...
    pkt->token_send = 0;
    pkt->type = 0;
    pkt->next_highest_pro_rate = 0;
    pkt->next_second_best_rate = 0;
...
void test_packet_serialize_maximum() {
    ...
    pkt->type = UINT8_MAX;
    pkt->next_highest_pro_rate = UINT8_MAX;
    pkt->next_second_best_rate = UINT8_MAX;
...
void test_packet_deserialize_normal() {
    return; //TODO, change input for activating this test case
    ...
    uint32_t len = 321;
    packet_t *pkt = packet_deserialize(input, 321 - getHeaderSize()); //TODO 321 is wrong
