/**
 * @file testing.c
 * @author your name (you@domain.com)
 * @brief 
 * @version 0.1
 * @date 2022-05-10
 * 
 * @copyright Copyright (c) 2022
 * 
 * For crude testing
 */

#include <assert.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "packet_minstrel.h"

static void test_packet_minstrel_build() {
    packet_minstrel_t* pkt = packet_minstrel_create(1, 2, 3, 4, 5, 6, 7, 8, 9);
    assert(pkt->id == 1);
    assert(pkt->next_symbol_rate == 2);
    assert(pkt->p_payload == 0x0);

    packet_minstrel_destroy(pkt);
}

static void test_packet_minstrel_destroy() {
    packet_minstrel_destroy(NULL);    
}

static void test_packet_minstrel_checksum() {
    packet_minstrel_t* pkt = malloc(sizeof(packet_minstrel_t));
    pkt->ack = 1;
    pkt->fallback_rate = 2;
    pkt->id = 65000;
    pkt->next_symbol_rate = 3;
    pkt->payload_len = 300;
    pkt->token_recv = 250;
    pkt->token_send = 400;
    pkt->type = 5;

    //mock payload data:
    pkt->p_payload = malloc(sizeof(uint8_t) * pkt->payload_len);
    for (int i=0;i<pkt->payload_len;i++) {
        pkt->p_payload[i] = 1;
    }
    packet_minstrel_set_checksum(pkt);

    assert (pkt->checksum == (65961 + 300) % 256); //Manual calculated
    assert (pkt->checksum != packet_minstrel_calc_checksum(pkt)-1);

    packet_minstrel_destroy(pkt);

    pkt = malloc(sizeof(packet_minstrel_t));
    pkt->ack = 1;
    pkt->fallback_rate = 1;
    pkt->id = 1;
    pkt->next_symbol_rate = 1;
    pkt->payload_len = 1;
    pkt->token_recv = 1;
    pkt->token_send = 1;
    pkt->type = 1;
    pkt->checksum = 0;
    pkt->p_payload = NULL;

    packet_minstrel_set_checksum(pkt);
    
    assert (pkt->checksum == 8); //Manual calculated
    assert (pkt->checksum != packet_minstrel_calc_checksum(pkt)-1);

    free(pkt);
}

//Helper function for test_packet_minstrel_serialize()
static void testSerializedPacket(packet_minstrel_t* pkt, uint8_t* serialized) {
    uint32_t count = 0;
    //build id from uint8_t
    uint32_t id = serialized[count] | serialized[count+1] << 8 | serialized[count+2] << 16 | serialized[count+3] << 24;
    count += 4;
    assert(id == pkt->id);

    uint8_t next_symbol_rate = serialized[count];
    count += 1;
    assert(next_symbol_rate == pkt->next_symbol_rate);

    uint16_t token_recv = serialized[count] | serialized[count+1] << 8;
    count += 2;
    assert(token_recv == pkt->token_recv);

    uint16_t token_send = serialized[count] | serialized[count+1] << 8;
    count += 2;
    assert(token_send == pkt->token_send);

    uint8_t type = serialized[count];
    count += 1;
    assert(type == pkt->type);

    uint32_t payload_len = serialized[count] | serialized[count+1] << 8 | serialized[count+2] << 16 | serialized[count+3] << 24;
    count += 4;
    assert(payload_len == pkt->payload_len);
    
    uint8_t checksum = serialized[count];
    count += 1;
    assert(checksum == pkt->checksum);

    uint32_t ack = serialized[count] | serialized[count+1] << 8 | serialized[count+2] << 16 | serialized[count+3] << 24;
    count += 4;
    assert(ack == pkt->ack);
    
    uint8_t fallback_rate = serialized[count];
    count += 1;
    assert(fallback_rate == pkt->fallback_rate);

    //check payload:
    for (int i=0;i<payload_len;i++) {
        assert(serialized[count + i] == pkt->p_payload[i]);
    }
}

static void test_packet_minstrel_serialize_normal() {
    //"normal" case
    packet_minstrel_t* pkt = malloc(sizeof(packet_minstrel_t));
    pkt->ack = 1;
    pkt->fallback_rate = 2;
    pkt->id = 65000;
    pkt->next_symbol_rate = 3;
    pkt->payload_len = 300;
    pkt->token_recv = 250;
    pkt->token_send = 400;
    pkt->type = 5;

    //mock payload data:
    pkt->p_payload = malloc(sizeof(uint8_t) * pkt->payload_len);
    for (int i=0;i<pkt->payload_len;i++) {
        pkt->p_payload[i] = i % 256;
    }
    packet_minstrel_set_checksum(pkt);

    uint32_t size = packet_minstrel_get_size(pkt);
    uint8_t* serialized = calloc(sizeof(uint8_t), size);
    
    assert(packet_minstrel_get_size(pkt) == (getHeaderSize() + pkt->payload_len) );

    packet_minstrel_serialize(pkt, serialized);

    testSerializedPacket(pkt, serialized);

    packet_minstrel_destroy(pkt);
}

static void test_packet_minstrel_serialize_zero() {

    packet_minstrel_t* pkt = malloc(sizeof(packet_minstrel_t));
    pkt->ack = 0;
    pkt->fallback_rate = 0;
    pkt->id = 0;
    pkt->next_symbol_rate = 0;
    pkt->payload_len = 1;
    pkt->token_recv = 0;
    pkt->token_send = 0;
    pkt->type = 0;

    //mock payload data:
    pkt->p_payload = malloc(sizeof(uint8_t) * pkt->payload_len);
    for (int i=0;i<pkt->payload_len;i++) {
        pkt->p_payload[i] = i % 256;
    }
    packet_minstrel_set_checksum(pkt);

    uint32_t size = packet_minstrel_get_size(pkt);
    uint8_t* serialized = calloc(sizeof(uint8_t), size);
    
    assert(packet_minstrel_get_size(pkt) == (getHeaderSize() + pkt->payload_len) );

    packet_minstrel_serialize(pkt, serialized);

    testSerializedPacket(pkt, serialized);

    packet_minstrel_destroy(pkt);
}

static void test_packet_minstrel_serialize_maximum() {

    packet_minstrel_t* pkt = malloc(sizeof(packet_minstrel_t));
    pkt->ack = UINT32_MAX;
    pkt->fallback_rate = UINT8_MAX;
    pkt->id = UINT32_MAX;
    pkt->next_symbol_rate = UINT8_MAX;
    pkt->payload_len = 256;
    pkt->token_recv = UINT16_MAX;
    pkt->token_send = UINT16_MAX;
    pkt->type = UINT8_MAX;

    //mock payload data:
    pkt->p_payload = malloc(sizeof(uint8_t) * pkt->payload_len);
    for (int i=0;i<pkt->payload_len;i++) {
        pkt->p_payload[i] = i % 256;
    }
    packet_minstrel_set_checksum(pkt);

    uint32_t size = packet_minstrel_get_size(pkt);
    uint8_t* serialized = calloc(sizeof(uint8_t), size);
    
    assert(packet_minstrel_get_size(pkt) == (getHeaderSize() + pkt->payload_len) );

    packet_minstrel_serialize(pkt, serialized);
    
    testSerializedPacket(pkt, serialized);

    packet_minstrel_destroy(pkt);
}

static void test_packet_minstrel_deserialize_normal() {
    //same case as in serialize_normal
    uint8_t input[] = {0xe8, 0xfd, 0x0, 0x0, 0x3, 0xfa, 0x0, 0x90, 0x1, 0x5, 0x2c, 0x1, 0x0, 0x0, 0xdb, 0x1, 0x0, 0x0, 0x0, 0x2, 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, , 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x0};
    uint32_t len = 321;
    packet_minstrel_t *pkt = packet_minstrel_deserialize(input);
    
    assert(pkt->id == 65000);
    assert(pkt->next_symbol_rate == 3);
    assert(pkt->token_recv == 250);
    assert(pkt->token_send == 400);
    assert(pkt->type == 5);
    assert(pkt->payload_len == 300);
    assert(pkt->checksum == 0xdb);
    assert(pkt->ack == 1);
    assert(pkt->fallback_rate == 2);

    //check content
    for (int i=0;i<pkt->payload_len;i++) {
        assert(pkt->p_payload[i] == input[getHeaderSize() - 1 + i]);
    }
}

static void test_packet_minstrel_deserialize_maximum() {
    uint8_t input[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0, 0x1, 0x0, 0x0, 0x79, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, , 0x0};
    uint32_t len = 277;
    packet_minstrel_t *pkt = packet_minstrel_deserialize(input);
    
    assert(pkt->id == UINT32_MAX);
    assert(pkt->next_symbol_rate == UINT8_MAX);
    assert(pkt->token_recv == UINT16_MAX);
    assert(pkt->token_send == UINT16_MAX);
    assert(pkt->type == UINT8_MAX);
    assert(pkt->payload_len == 256);
    assert(pkt->checksum == 0x79);
    assert(pkt->ack == UINT32_MAX);
    assert(pkt->fallback_rate == UINT8_MAX);

    //check content
    for (int i=0;i<pkt->payload_len;i++) {
        assert(pkt->p_payload[i] == input[getHeaderSize() - 1 + i]);
    }
}


int main(int argc, char** argv) {
    if (argc < 2) { //there are no options
        printf("No options which packet should be tested in arguments\n");
        return 1;
    }
    for (int i=0;i<argc;i++) {
        printf("Test run[%i]: %s\n", i, argv[i]);
    }
    if (!strcmp(argv[1], "packet.minstrel.build")) {
        printf("Start minstrel build\n");
        test_packet_minstrel_build();
    } else if (!strcmp(argv[1], "packet.minstrel.destroy"))  {
        printf("Start minstrel destroy\n");
        test_packet_minstrel_destroy();
    } else if (!strcmp(argv[1], "packet.minstrel.checksum")) {
        printf("Start minstrel checksum\n");
        test_packet_minstrel_checksum();
    } else if (!strcmp(argv[1], "packet.minstrel.serialize.normal")) {
        printf("Start minstrel serialize normal\n");
        test_packet_minstrel_serialize_normal();
    } else if (!strcmp(argv[1], "packet.minstrel.serialize.zero")) {
        printf("Start minstrel serialize zero case\n");
        test_packet_minstrel_serialize_zero();
    } else if (!strcmp(argv[1], "packet.minstrel.serialize.maximum")) {
        printf("Start minstrel serialize maximum case\n");
        test_packet_minstrel_serialize_maximum();
    } else if (!strcmp(argv[1], "packet.minstrel.deserialize.normal")) {
        printf("Start minstrel deserialize normal case\n");
        test_packet_minstrel_deserialize_normal();
    } else if (!strcmp(argv[1], "packet.minstrel.deserialize.maximum")) {
        printf("Start minstrel deserialize maximum case\n");
        test_packet_minstrel_deserialize_maximum();
    } else {
        printf("No valid test command.\n");
        return 1;
    }

    return 0;
}